...
//    ADLdp_thr = MIN_ADLP + ADLdp;
    ...
    if (BT_OK)
    {
        ...
        while (BT_OK) {};                       //ждать поки нажата кнопка
    ...
    while (1)
    {
        ...
        switch (KeyPress)                                   //обробка клавіш по їх функціях
        {
            ...
            {
                isChng = 1;                                 //відбулась зміна значення
                m_delay = 5*CPS0_S;                         //затримка поточного режиму 5sec
                ...
                switch (StMode)
                {
                    ...
                    case SM_MAIN_SEC:
                    {
                        ...
                        if (((StMode == SM_MAIN_PRM)&&(ePCM == PCM_TEMP))||((StMode == SM_MAIN_SEC)&&(ePCM == PCM_PRST)))
                        {
                            ...
                            if (DispVal > MIN_TEMP) DispVal -= 5;   //температура на екрані більша за мінімальну то зменшуємо її
                            else beep(50);
                        }
            ...
            case ENC_R:                                     //Plus - все аналогічно "мінусу"
            {
                isChng = 1;
                m_delay = 5*CPS0_S;
                BlinkDelay = CPS0_S/4;                      //затримка мигання 0.25sec
            ...
            case OK_SHT:                        //Ok short
            {
                ...
                {
                    LngKeyPress = 0;                            //скидання довгого утримування кнопки
                                                                //(блокування короткого натискання при відпусканні довгого)
                    KeyPress = 0;                               //скидання функції натиснутої кнопки
                    break;
                }
                switch (StMode)
                {
                    case SM_MAIN_PRM:
                    {
                        m_delay = 4*CPS0_S;     //затримка виходу з поточного режиму    4sec
                        isChng = 2;
                    ...
                    {
                        m_delay = 4*CPS0_S;     //затримка виходу з поточного режиму    4sec
                        //isChng = 2;             //перехід в інший режим
                        ...
                        //isChng = 1;
                        StMode = SM_TMR_SET;    //переходим в режим налаштування теймера
                        DispVal = e_tval;
                        beep(50);
                        break;
                    };
...
//                            StMode = SM_MAIN_PRM;
        ...
        switch (StMode)
        {
            ...
            case SM_MAIN_PRM:
            {
                ...
                else
                {
                    ...
                    isStab = 0;
                    StHyst = HT_LCK;
                    if (StMode == SM_MAIN_PRM) r_led();         //----!!!!!!!!!!!!!!!!!! подумать !!!!!!!!!!!
                }
                //--------------------------------------------------------
                break;
            };
        ...
        switch (StMode)                                             //вибір режиму станції
        {
            case SM_MAIN_PRM:
            {
                if (/*isSS ||*/ isChng)
                {
                    enBlk = 3;                                      //мигання увімкнуто для 3х розрядів
                    if (!isChng) DispVal = ADCDispVal;
                }
                else
                {
                    enBlk = 0;                                      //мигання вимкнуто
                    DispVal = ADCDispVal;
                }
                ReCode(DispVal, 3);
                break;
            ...
            case SM_MAIN_SEC:
            {
                enBlk = 3;                                          //мигання увімкнуто для 2х розрядів
                ReCode(DispVal, 3);
                break;
            };
...
{
    static byte stateEnc; 	//хранит последовательность состояний энкодера
